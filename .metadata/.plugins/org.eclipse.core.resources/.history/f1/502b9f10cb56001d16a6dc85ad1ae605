...
public class UI 
{
	...
	public void drawInventario()
	{
		...
		final int janelaLargura = gp.tileSize*10;
		final int janelaAltura = gp.tileSize*7;
		
		g2.drawImage(gp.player.inventario, janelaX, janelaY, janelaLargura, janelaAltura+20, null);
		
		int textoX = janelaX + 45;
		...
		g2.drawString(valores, textoX-285, textoY);
		textoY = textoY + alturaLinha;
		
		valores = String.valueOf(gp.player.vida);
		textoX = textoCentralizadoInventario(valores, valorX);
		g2.drawString(valores, textoX-300, textoY);
		textoY = textoY + alturaLinha;
		
		valores = String.valueOf(gp.player.moeda);
		textoX = textoCentralizadoInventario(valores, valorX);
		g2.drawString(valores, textoX-255, textoY);
		textoY = textoY + alturaLinha;
